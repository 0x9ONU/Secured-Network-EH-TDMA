...
const unsigned int TROJAN_OFFSET = 80;                      // Amount of time added to clock by Trojan in ms
  ...
  switch(state) {
    ...
    case SYNC: {
      ...
      if(myFlags.is_sync || myFlags.is_data) {
        ...
        myFlags.offset = (myFlags.global_time - (long) myFlags.time_in) + TROJAN_OFFSET; //Trojan attacks and adds time to offset
        if(myFlags.offset < 0) {
          myFlags.offset = (long) CYCLE_LENGTH + myFlags.offset + TROJAN_OFFSET; //Trojan attacks and adds time to offset
        }
